...
def test_preparation_keeps_only_required_paired_raw_data(
    ...
    """Preparation writes paired raw counts without redundant metadata."""
    ...
    adata = ad.AnnData(
        ...
        obs=obs,
        var=pd.DataFrame(index=["GENE_A", "GENE_B", "UNMAPPED"]),
        layers={
    ...
    assert prepared.var_names.tolist() == ["ENSG_A", "ENSG_B"]
    ...
    np.testing.assert_array_equal(prepared.X.toarray(), raw_counts[:2, :2])
