...
def main() -> None:
    ...
    tgt_datasets = read_dataset_files(
        config["data"]["tgt_dataset_folder"], "dataset"
    )
    if (
        "tgt_vocab_size" in config["trainer"]
        and "max_seq_length" in config["trainer"]
    ):
    ...
    if "perturb_genes_file" in config["data"]:
        perturb_genes_df = pd.read_csv(
            config["data"]["perturb_genes_file"], header=0
        )
        ...
        if (
            "perturb_cluster" in config["data"]
            and "perturb_colname" in config["data"]
        ):
    ...
    if "output_dir" in config["trainer"]:
        ...
        for gene in genes_to_perturb:
            pattern = re.compile(
                r"adata_g" + re.escape(gene) + r"_ssrc", re.IGNORECASE
            )
        ...
        raise ValueError(
            "output_dir must be provided in trainer in the config file"
        )
    ...
    if len(config["trainer"]["genes_to_perturb"]) > 0:
        ...
        if "cond_list" in config["data"]:
            full_dataset = concatenate_datasets(
                [src_dataset] + list(tgt_datasets.values())
            )
            condition_dict = {}
        ...
        tgt_adata_tmp = tgt_adatas[
            f"tgt_h5ad_t{config['trainer']['pred_tps'][0]}"
        ].copy()
        condition_keys = (
            config["data"]["condition_keys"]
            if "condition_keys" in config["data"]
            else None
        )
        conditions_combined = (
        ...
        if ("filter_var" in config["data"]) and (
            "filter_cond" in config["data"]
        ):
        ...
        if config["model"]["ckpt_masking_path"] is not None:
            ...
            if config["model"]["ckpt_masking_path"].endswith(".bin"):
                ...
                if len(unexpected) > 1:
                    raise Warning(
                        f"Unexpected keys in state_dict: {unexpected}"
                    )
                trainer.test(
                    decoder_module,
                    data_module,
